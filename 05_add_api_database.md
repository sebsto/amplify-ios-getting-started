...
- *? Do you want to edit the schema now?*, accept the default *Y* and press **enter**
    ...
    The default text editor that your choose when you initialized the project (`amplify init`) opens with a prebuilt data schema.  
    ...
    **Delete** the schema and **replace it** with our app GraphQL schema:
    ...
    The data mode is made of one class `NoteData` and 4 properties : `id` and `name` are mandatory. `description` and `image` are optional Strings.

    The `@model` transfomer indicates we want to create a database to store these data.

    The `@auth` transformer indicates authentication rules to allow access to these data. For this project, we want only the owner of `NoteData`s to have access to them.
...
This created Swift files in `amplify/generated/models` directory, as you can see with:
...
Back to Xcode, open `Backend` and add a line in the Amplify initialisation sequence in `private init()`. Complete code block should lool like this:
...
Let's add code to call our API: a method to query the Note, a methof to create a new Note, and a method to delete a Note. Notice that these method works on the app data model (`Note`) to make it easy to interract from the User Interface. These method transparently convert `Note` to `NoteData` objects.
...
At the launch of the app, we must call the API to query the list of `Note` for the currently signed in user. **Add** this piece of code in the `Backend`'s `private init()` method:
...
Now, we just need to create a piece of user interface to create a new `Note` and to delete a `Note` from the list.
...
Finally, in `ContentView`, add the 'swipe to delete' behaviour
...
| Empty List | Create a Note | One Note in the List |
| --- | --- | --- |
| ![Empty List](img/05_20.png) | ![Create Note](img/05_30.png) | ![One Note in the List](img/05_40.png)
